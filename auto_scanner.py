        ...
        try:
            ...
            if os.name == 'nt':
                ...
                if str(old_pid) in out and "python" in out.lower():
                    print(f"AutoScanner is already running locally with PID {old_pid}. Exiting to prevent duplication.")
                    sys.exit(0)
            else:
                # Unix check: send signal 0 to verify process existence
                try:
                    os.kill(old_pid, 0)
                    print(f"AutoScanner is already running with PID {old_pid}. Exiting.")
                    sys.exit(0)
                except OSError:
                    pass
        except Exception:
